...
def DriverLogin(request):
    ...
    if a.exists():
        ...
        if b.type=="Driver":
            return JsonResponse({"status":"ok","type":b.type,"lid":b.id})
        elif b.type=="User":
            return JsonResponse({"status":"ok","type":b.type,"lid":b.id})
        else:
...
def driver_Signup(request):
    ...
    fh1 = open("C:\\Users\\jishn\\OneDrive\\Desktop\\velogo\\VeloGO\\media\\Licence\\" + date + "-1.jpg", "wb")
    # fh = open("C:\\Users\\91815\\PycharmProjects\\cyber\\media\\" + date + ".jpg", "wb")
    path1 = "/media/License/" + date1 + "-1.jpg"
    fh1.write(a1)
    fh1.close()
    b = Login()
    b.Username = email
...
def Driver_edit_profile_post(request):
    ...
    if len(photo)>0:
        ...
        date = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
        a = base64.b64decode(photo)
        fh = open("C:\\Users\\jishn\\OneDrive\\Desktop\\velogo\\VeloGO\\Velo_Go\\media\\driver\\" + date + ".jpg", "wb")
        # fh = open("C:\\Users\\91815\\PycharmProjects\\cyber\\media\\" + date + ".jpg", "wb")
        path = "/media/" + date + ".jpg"
        fh.write(a)
        fh.close()
        res=Drivers.objects.filter(LOGIN=lid).update(name=name,dob=DateofBirth,gender=gender,phone=phone,email=email,photo=path,place=place,post=post,pin=pin,state=state,
                                                    city=city,lisence=lisence,upi=upi,languagesknown=languagesknown)
        return JsonResponse({'status':"ok"})

    else:
        res=Drivers.objects.filter(LOGIN=lid).update(name=name,dob=DateofBirth,gender=gender,phone=phone,email=email,place=place,post=post,pin=pin,state=state,
                                                     city=city, lisence=lisence, upi=upi, languagesknown=languagesknown)
        return JsonResponse({'status':"ok"})
...
def AddVehicle(request):
    ...
    d.photo=path
    d.PollusionCertificate=PollusionCertificate
    d.rc=rc
...
def DriverViewVehicle(request):
    lid=request.POST["lid"]
    e=Vehicle.objects.filter(LOGIN_id=lid)
...
def Driver_Add_Seat(request):
    ...
    a.VEHICAL=Vehicle.objects.get(DRIVERS__LOGIN_id=lid)
    return JsonResponse({'status': "ok"})

def Driver_view_seats(request):
    lid = request.POST["lid"]
    var=Seats.objects.filter(DRIVERS__LOGIN_id=lid)
    l=[]
    for i in var:
...
def Driver_edit_seats(request):
    lid = request.POST["lid"]
    i=Seats.objects.get(VEHICAL__DRIVERS__LOGIN_id=lid)

    l = []
    l.append(
        {"id": i.id, 'naNumberOfSeatsme': i.NumberOfSeats, 'VaccantSeats': i.VaccantSeats,})
    return JsonResponse({"status": "ok", 'data': l})
...
def Driver_view_allocated_booking(request):
    ...
    l=[]
    for i in a:
        l.append({"id":i.id,"SEATS":i.SEATS.NumberOfSeats,"Date":i.Date,"time":i.time,"Destination":i.Destination,
                  "PickupPoint":i.PickupPoint,"PickupDate":i.PickupDate,"PickupTime":i.PickupTime,"EstimatedPrice":i.EstimatedPrice,
                  "EstimatedRideTime":i.EstimatedRideTime})
        return JsonResponse({'status': "ok",'data':l})
...
def Driver_view_Passengers_And_DropPoints(request):
    ...
    l=[]
    for i in a:
        l.append({"id":i.id,"name":i.USERS.Name,"PickupPoint":i.USERS,"Destination":i.USERS,"PickupDate":i.USERS,
                  "PickupTime":i.USERS,"EstimatedPrice":i.USERS,"EstimatedRideTime":i.USERS})
        return JsonResponse({'status': "ok", 'data': l})
...
def Driver_view_previous_booking(request):
    ...
    for i in a:
        l.append(
            {"id": i.id, "name": i.USERS.Name, "PickupPoint": i.USERS, "Destination": i.USERS, "PickupDate": i.USERS,
             "PickupTime": i.USERS, "EstimatedPrice": i.USERS, "EstimatedRideTime": i.USERS})
        return JsonResponse({'status': "ok", 'data': l})

def Driver_view_rating(request):
    lid = request.POST["lid"]
    a=Reviews.objects.filter(DRIVERS__Rating__LOGIN_id=lid)
    l=[]
    for i in a:
        l.append({"id":i.id,"name":i.USERS.Name,"Review":i.Review,"Date":i.Date,})
        return JsonResponse({'status': "ok", 'data': l})
